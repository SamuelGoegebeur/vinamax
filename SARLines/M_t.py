...
def M(df,x='H'):
    ...
    if x=='t':
        fig, ax = plt.subplots()
        ax.plot(df["#t"]*10**6,df['<mz>'],color='red',label='M')
    ...
    if x=='tH':
        ...
        ax.plot(df["#t"]*10**6,df['B_ext_z']*10**3,color='blue',label = 'B')
        ...
        ax2 = plt.twinx(ax)
        ax2.plot(df["#t"]*10**6,df['<mz>'],color='red',label='M')
        ax2.set_ylim(-1,1)
        ...
        ax3.plot(df['B_ext_z'][:]*10**3,df['<mz>'][:])
